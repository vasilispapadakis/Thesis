...
class LupiRegressor(BaseEstimator, TransformerMixin):
    ...
    def _decfun (self, X_train_all, y_train_all):
        ...
        skf = StratifiedKFold(n_splits=self.cv, shuffle=False, random_state=None)

        estimator = SVC(class_weight='balanced', kernel='linear', random_state=1,decision_function_shape='ovr')
    ...
    def _train_regr(self, X_train, y_train):
        ...
        for j in np.arange(X.shape[1]- num_of_std):
            ...
            if regr_type == 'kernelridge':
                regr = GridSearchCV(KernelRidge(kernel='linear'),
...
class Composer():
    ...
    def _fit_estimator(self, X_train, y_train, lupiregr_ps):
        lupi_estimator = LupiSvmClassifier(lupiregressor=lupiregr_ps, class_weight='balanced',
                                           kernel='linear', random_state=self.random_state)
    ...
    def fit(self, X_train, y_train):
        ...
        if len(self.pri_features) == 0:
            ...
            estimator = SVC(class_weight='balanced', kernel='linear', random_state=self.random_state,decision_function_shape='ovr')
